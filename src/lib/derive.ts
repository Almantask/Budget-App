...
import { categoryBreakdown, fullHistory, monthlySeries, weeklySeries } from './analytics.ts'
...
export function deriveView(state: AppState, viewMonth: string, now = new Date()) {
  ...
  const series = monthlySeries(state.transactions, viewMonth, 12)
  ...
  const achievements = collectAchievements({ state, today, history, hits })
